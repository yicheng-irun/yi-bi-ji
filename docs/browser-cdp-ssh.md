...
- **Linux 上 `curl 127.0.0.1:9222` 报 `Empty reply from server`（不是 Connection refused）**：说明 Linux 上 9222 有监听（通常是隧道），但隧道另一端（浏览器 CDP）没真正提供服务。先在浏览器所在电脑上执行 `curl http://127.0.0.1:9222/json/version`：
  - 它也失败 → 浏览器没起来 / 调试端口没开：确认是用 `start-browser-cdp.bat` 启动的窗口确实打开了；若之前开过不带调试参数的 Edge，先关掉所有 `.biji-browser` 配置目录的进程再重跑（`netstat -ano | findstr :9222` 可确认监听）。
  - 它正常返回 JSON → 隧道目标没接上：先启动浏览器，再重新建立隧道（`-R` 建隧道时不会预检目标端口），可加 `-o ExitOnForwardFailure=yes` 以便绑定失败立即报错。
